...
class FeLine(Engine):
    ...
    def cmd_handle(self):
        user_input = self.prompt.multiline_prompt
...
if __name__ == '__main__':
    ...
    # process arguments
    parser = ArgParser()
    parser.set_arg_models(
        feline.client.AVAILABLE_MODELS,
        feline.client.DEFAULT_MODEL
    )
    ...
    if args.message or args.interactive:
        ...
        feline.prompt.multiline_prompt = args.message
        feline.prompt.interactive = args.interactive
        feline.client.model = feline.client.AVAILABLE_MODELS[args.model]
